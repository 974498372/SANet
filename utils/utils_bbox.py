...
def dba_correct_boxes(box_xy, box_wh, input_shape, image_shape, letterbox_image):
...
def decode_outputs(outputs, input_shape):
    ...
    #   Obtain the probability that each feature point belongs to each category
...
def non_max_suppression(prediction, num_classes, input_shape, image_shape, letterbox_image, conf_thres=0.5, nms_thres=0.4):
    ...
    for i, image_pred in enumerate(prediction):
        ...
        if output[i] is not None:
            ...
            output[i][:, :4]    = dba_correct_boxes(box_xy, box_wh, input_shape, image_shape, letterbox_image)
    return output
